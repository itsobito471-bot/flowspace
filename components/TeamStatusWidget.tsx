...
export default function TeamStatusWidget() {
  ...
  const fetchTeam = async () => {
    ...
    try {
      ...
      // Sort so online users fall to the front
      const sortedTeam = json.data.sort((a: TeamMember, b: TeamMember) => {
  ...
  useEffect(() => {
    fetchTeam();
    // Auto Refresh every 60 seconds ensuring dynamic presence sensing
  ...
  return (
    <>
      ...
      <div className="bg-surface border border-muted/10 rounded-3xl p-6 shadow-xl relative overflow-hidden">
        {/* Slight ambient gradient behind the widget */}
        ...
        <div className="flex justify-between items-center mb-6 relative z-10">
          <div>
            ...
            <p className="text-[10px] text-muted tracking-[0.2em] uppercase mt-0.5">Who's at the command center</p>
          </div>
          <button 
            onClick={fetchTeam} 
            disabled={loading} 
